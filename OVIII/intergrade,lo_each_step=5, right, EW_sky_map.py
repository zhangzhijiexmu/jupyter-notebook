"""
    1、运行完本程序即可得到文件'intergrade,lo_each_step=5, right, OVIII_column_number_density_integrate.txt',
    2、然后再运行程序"intergrade,lo_each_step=5, right, all_sky_map.py" 即可得到OVIII的all-sky-map 图片'integrate,lo_each_step=5, right, OVIIIColumn density integrate colormap.pdf'
    """
...
for l,l_location in zip(longitude,longitude_length_list):
    ...
    for b,b_location in zip(latitude,latitude_length_list) :
        ...
        wavelength = 18.9689e-8 #OVIII的波长
        v12 = CLIGHT/wavelength #OVIII的频率
        

        

       
        optical_depth = optical(nOVIII,Doppler_T,Radial_velocity1,Radius)
        flux =np.exp(-optical_depth)
        

        dv=0.0001e17

        V = np.arange(1.57e+17,1.59e+17,dv)
        E=h*V
        ...
        lam, fosc = 18.9689, 4.16e-1  #OVIII波长以艾米为单位，和振子强度(为文档Parameters of Lines.pdf中的fik)
        gam = obgamma(lam, 2, 6, fosc)  #calculate the Einstein A coefficent
        ...
        mod = Model(Abvoigt)
        para = mod.make_params(lam=lam, bpar=100, logn=14.5, z=0, fosc=fosc, gam=gam)
        ...
        OVIII_column_number_density[l_location,b_location]=out.best_values['logn'] #得到最佳拟合值OVIII柱密度的对数值
        redshit= out.best_values['z']
        
        #利用得到的最佳拟合值bpar、logn再次带入voigt进行计算得到拟合曲线的光深。
        flux_fitting=Abvoigt(x=wave, lam=lam, bpar=Doppler[l_location,b_location], logn=OVIII_column_number_density[l_location,b_location], z=redshit, fosc=fosc, gam=gam)
        
        Equivalent_width[l_location,b_location]=(-integrate.trapz(1-flux_fitting,wave))*1e3 # 负号是因为积分波长间隔为负数，*1e3是以毫艾为单位。
        """
        print("longitude=%d,lattitude=%d,OVIII_column_number_density_integrate is %f"%(l,b,OVIII_column_number_density_integrate[l_location, b_location]))





np.savetxt('intergrade,lo_each_step=5, right, OVIII_column_number_density_integrate.txt',OVIII_column_number_density_integrate)

#np.savetxt('lo_each_step=5, right, Equivalent_width.txt',Equivalent_width) #

#np.savetxt('lo_each_step=5, right, OVIII_column_number_density.txt',OVIII_column_number_density)

#np.savetxt('lo_each_step=5, right, H_gas_column_number_density.txt',H_column_number_density)
#np.savetxt('lo_each_step=5, right, Doppler.txt',Doppler)
...
print("总运行时间:%f hours"%((endtime-begintime)/3600))  #总运行时间:2.700962 hours
