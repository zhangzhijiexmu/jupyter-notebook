"""
    1、得先运行运行程序'intergrade,lo_each_step=5, right, EW_sky_map.py',即可得到文件'intergrade,lo_each_step=5, right, OVII_column_number_density_integrate.txt',
    2、然后再运行本程序即可得到OVII的all-sky-map 图片'integrate,lo_each_step=5, right, OVIIColumn density integrate colormap.pdf'
"""
...
"""
    ...
    data_l_274_OVII_column_number_density = np.hstack((np.loadtxt('274-274,b=-90~ -31OVII_column_number_density.txt'), np.loadtxt('274-274,b=-30~-30OVII_column_number_density.txt'), np.loadtxt('274-274,b=-29~ 90OVII_column_number_density.txt')) )
    
    
    np.savetxt('274-274OVII_column_number_density.txt',data_l_274_OVII_column_number_density)
    ...
    data_l_283_OVII_column_number_density = np.hstack((np.loadtxt('283-283,b=-90~-18OVII_column_number_density.txt'), np.loadtxt('283-283,b=-17~-17OVII_column_number_density.txt'), np.loadtxt('283-283,b=-16~90OVII_column_number_density.txt')) )
    
    
    np.savetxt('283-283OVII_column_number_density.txt',data_l_283_OVII_column_number_density)
...
for i in number:
    ...
    column_number_density.append(np.loadtxt('%s'%i+'OVII_column_number_density.txt') )
...
np.savetxt("OVII_column_number_density.txt",column)
...
OVII_column_number_density_integrate_data=np.loadtxt('intergrade,lo_each_step=5, right, OVII_column_number_density_integrate.txt')

OVII_column_number_density_integrate_data_lines=OVII_column_number_density_integrate_data.shape[0]

OVII_column_number_density_integrate_data_culumns=OVII_column_number_density_integrate_data.shape[1]




#把OVII_column_number_density_data按行顺序展开成一个列表，和下面的纬度经度展开成列表相对应。
OVII_column_number_density_integrate_data_list=[]

for i in range(0,OVII_column_number_density_integrate_data_lines):
    
    for j in range(0,OVII_column_number_density_integrate_data_culumns):
        
        OVII_column_number_density_integrate_data_list.append(OVII_column_number_density_integrate_data[i,j])
...
OVII_column_number_density_data=np.loadtxt('lo_each_step=5, right, OVII_column_number_density.txt')

OVII_column_number_density_data_lines=OVII_column_number_density_data.shape[0]

OVII_column_number_density_data_culumns=OVII_column_number_density_data.shape[1]
...
#把OVII_column_number_density_data按行顺序展开成一个列表，和下面的纬度经度展开成列表相对应。
OVII_column_number_density_data_list=[]

for i in range(0,OVII_column_number_density_data_lines):
    
    for j in range(0,OVII_column_number_density_data_culumns):
        
        OVII_column_number_density_data_list.append(OVII_column_number_density_data[i,j])
...
data_really=np.column_stack((longitude_list,latitude_list,OVII_column_number_density_integrate_data_list)) #把OVII_column_number_density_integrate_data_list加入到position的第三列中。

np.savetxt('intergrade,lo_each_step=5, right, data_really_list.txt',data_really)

"""
data=np.column_stack((longitude_list,latitude_list,EW_data_list,OVII_column_number_density_data_list,H_gas_column_number_density_data_list,Doppler_data_list)) #把density加入到position的第四列中。
...
#插值OVII_column_number_density_integrate
OVII_column_number_density_integrate_data_list=data[:,2]
values= OVII_column_number_density_integrate_data_list
...
OVII_column_number_density_integrate_data=grid_z1

OVII_column_number_density_integrate_data_lines= OVII_column_number_density_integrate_data.shape[0] #EW_data的行数

OVII_column_number_density_integrate_data_culumns= OVII_column_number_density_integrate_data.shape[1] #EW_data的列数
...
#插值OVII_column_number_density
OVII_column_number_density_data_list=data[:,3]
values= OVII_column_number_density_data_list
...
OVII_column_number_density_data=grid_z1

OVII_column_number_density_data_lines= OVII_column_number_density_data.shape[0] #EW_data的行数

OVII_column_number_density_data_culumns= OVII_column_number_density_data.shape[1] #EW_data的列数
...
data=np.column_stack((longitude_list,latitude_list,OVII_column_number_density_integrate_data_list)) #把Doppler_data_list加入到longitude的第六列中。
#data=np.column_stack((longitude_list,latitude_list,EW_data_list,OVII_column_number_density_data_list,H_gas_column_number_density_data_list,Doppler_data_list)) #把Doppler_data_list加入到longitude的第六列中。
...
##df = df.loc[:,['hp','EW_data_list','OVII_column_number_density_data_list','H_gas_column_number_density_data_list']].groupby(by=['hp']).mean().reset_index()
import healpy as hp
import numpy as np
import pandas as pd
import matplotlib.pyplot as plt
...
df = pd.DataFrame(columns = ['longitude_list','latitude_list','OVII_column_number_density_integrate_data_list'])
file = open('intergrade,lo_each_step=5, right, data_high_resolution.txt').readlines()
"""
df = pd.DataFrame(columns = ['longitude_list','latitude_list','EW_data_list','OVII_column_number_density_data_list','H_gas_column_number_density_data_list','Doppler_data_list'])
...
df.to_csv(r'./intergrade,lo_each_step=5, right, ovii.csv', index=False)


df = pd.read_csv('intergrade,lo_each_step=5, right, ovii.csv')
...
df = df.loc[:,['hp','OVII_column_number_density_integrate_data_list']].groupby(by=['hp']).mean().reset_index()
#df = df.loc[:,['hp','EW_data_list','OVII_column_number_density_data_list','H_gas_column_number_density_data_list','Doppler_data_list']].groupby(by=['hp']).mean().reset_index()
...
hp.mollview(
            df['OVII_column_number_density_integrate_data_list'].values,
            coord="G",
            #title = 'SFE1, snapshot_155, OVII Column density colormap',
            title = 'integrate, right, ALL SKY MAP',
            unit =  r'log$N_{OVII}$(cm$^{-2})$',
            cmap='jet'
            )
hp.graticule()

plt.savefig('integrate,lo_each_step=5, right, OVIIColumn density integrate colormap.pdf',format='pdf', dpi=1000)
...
hp.mollview(
    df['OVII_column_number_density_data_list'].values,
    coord="G",
    #title = 'SFE1, snapshot_155, OVII Column density colormap',
    title = 'lo_each_step=5, right, ALL SKY MAP',
            unit =  r'log$N_{OVII}$(cm$^{-2})$',
...
plt.savefig('lo_each_step=5, right, OVIIColumn density colormap.pdf',format='pdf', dpi=1000)
